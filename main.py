...
def main():
    ...
    try:
        # Load and trim main recording with progress bar
        ...
        # Add intro with progress bar
        if intro_path:
            print("2. Adding intro...")
            with tqdm(total=100, desc="Loading intro", unit="%", colour="blue") as pbar:
        ...
        # Add outro with progress bar
        if outro_path:
            print("3. Adding outro...")
            with tqdm(total=100, desc="Loading outro", unit="%", colour="cyan") as pbar:
                outro_clip = VideoFileClip(outro_path)
                pbar.update(100)
                clips_to_concat.append(outro_clip)
        
        # Concatenate all clips with progress bar
        print("4. Combining all clips...")
        with tqdm(total=100, desc="Combining clips", unit="%", colour="yellow") as pbar:
        ...
        with tqdm(total=100, desc="Exporting video", unit="%", colour="magenta") as pbar:
        ...
        # Clean up with progress bar
        print("6. Cleaning up...")
        with tqdm(total=100, desc="Cleaning up", unit="%", colour="red") as pbar:
            main_clip.close()
            if intro_path:
                intro_clip.close()
            if outro_path:
